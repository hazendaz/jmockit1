 ...
 * the initialization of an <code>Expectations</code> object. This is done by instantiating an anonymous subclass
 * containing an instance initialization body, or as we call it, an <em>expectation block</em>:
 *
 * <pre>{@code
 ...
 * will fail with a <code>MissingInvocation</code> error.
 * <p>
 * When multiple expectations are recorded, matching invocations are allowed to occur in a <em>different</em> order. So,
 ...
 * a <em>sequence</em> of values; b) argument matchers such as {@link #anyInt}, {@link #anyString},
 ...
 * <em>after</em> exercising the code under test. To that end, we use a set of complementary base classes:
 * {@link Verifications}, {@link VerificationsInOrder}, and {@link FullVerifications}. Similar to expectation blocks,
 * these classes allow us to create <em>verification</em> blocks.
 *
 * @see #Expectations()
...
public class Expectations extends Invocations {
     ...
     * <ul>
     * <li>Most <code>java.lang</code> types (<code>String</code>, <code>Object</code>, etc.): returns
     * <code>null</code>.</li>
     * <li><code>java.math</code> types (<code>BigDecimal</code>, etc.): returns <code>null</code>.</li>
     * <li>Primitive/wrapper types: returns the standard default value (<code>false</code> for
     * <code>boolean/Boolean</code>, <code>0</code> for <code>int/Integer</code>, and so on).</li>
     ...
     * <code>IllegalArgumentException</code> to be thrown, unless it can be safely converted to the return type. One
     ...
     * the field. This applies to <code>void</code> and non-<code>void</code> methods, as well as to constructors.
     *
     ...
     * <ol>
     * <li>If the return type is iterable and can receive a {@link List} value, then the given sequence of values will
     * be converted into an <code>ArrayList</code>; this list will then be returned by matching invocations at replay
     * time.</li>
     * <li>If the return type is <code>SortedSet</code> or a sub-type, then the given sequence of values will be
     * converted into a <code>TreeSet</code>; otherwise, if it is <code>Set</code> or a sub-type, then a
     * <code>LinkedHashSet</code> will be created to hold the values; the set will then be returned by matching
     * invocations at replay time.</li>
     * <li>If the return type is <code>Iterator</code> or a sub-type, then the given sequence of values will be
     * converted into a <code>List</code> and the iterator created from this list will be returned by matching
     * invocations at replay time.</li>
     ...
     * specified to be returned. This upper limit can be overridden through the <code>maxTimes</code> field, if
     * necessary.
     *
     * @param firstValue
